      ...
      <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <div className="space-y-4">
            {['card','upi','wallet'].map((id) => {
              const icons: any = {
                card: <CreditCard className="w-5 h-5" />,
                upi: <Landmark className="w-5 h-5" />,
              ...
              return (
                ...
                >
                  ...
                  {method === id && <CheckCircle className="ml-auto w-5 h-5 text-brand-cyan" />}
                </button>
              );
            })}
          </div>
          ...
          <GlassCard className="p-8 h-full flex flex-col justify-between border border-white/10" gradient>
            {!bookingId ? (
              <div className="text-center text-red-400">
                <p>No active booking found. Redirecting to trip planner...</p>
              </div>
            ) : (
              <>
                <div>
